...
const getThemeDictionary = () => {
  ...
  const { themes } = chainConfig.style;

  for (let i = 0; i < themes.length; i += 1) {
    const {
      type, background, primary, divider, text, fonts,
    ...
    const theme:ThemeOptions = R.mergeDeepLeft(ThemeOverride, common);
    themeDict[type] = theme;
  }

  return themeDict;
};

export const themeDictionary = getThemeDictionary();
